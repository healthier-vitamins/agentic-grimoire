# AGENTS.md — agentic-grimoire guidelines
...
context stays clean; the cost win comes from that isolation, not from starving the
worker's reasoning. Match each subagent's reasoning effort to its task — low for a
simple lookup, higher for interdependent changes or debugging — and reach for more
